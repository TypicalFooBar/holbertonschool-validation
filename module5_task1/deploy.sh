...
# Read the tag name to use for the docker image
docker_tag_name=$(cat awesome_image_tag_name.txt)
...
ssh -o StrictHostKeyChecking=accept-new ubuntu@"$new_ec2_instance_public_ip" "
	...
	docker run --detach -p 80:9999 -p 443:9999 --restart='always' awesome:$docker_tag_name
"